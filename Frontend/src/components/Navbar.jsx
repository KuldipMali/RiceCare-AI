import { Link, NavLink } from "react-router";

const Navbar = () => {

  const linkStyle = ({ isActive }) =>
    isActive
      ? "text-primary font-semibold border-b-2 border-primary pb-1"
      : "text-gray-600 hover:text-primary transition";



  return (
    <nav className="w-full z-20 fixed top-0 h-25 bg-light flex items-center px-20 justify-between">
      <div className="left flex items-center gap-2">
        <svg
          className="text-white bg-primary p-2 rounded-xl shadow-sm shadow-secondary"
          xmlns="http://www.w3.org/2000/svg"
          width={40}
          viewBox="0 0 24 24"
          fill="currentColor"
        >
          <path d="M20.998 3V5C20.998 14.6274 15.6255 19 8.99805 19L7.0964 18.9999C7.3079 15.9876 8.24541 14.1648 10.6939 11.9989C11.8979 10.9338 11.7965 10.3189 11.2029 10.6721C7.1193 13.1016 5.09114 16.3862 5.00119 21.6302L4.99805 22H2.99805C2.99805 20.6373 3.11376 19.3997 3.34381 18.2682C3.1133 16.9741 2.99805 15.2176 2.99805 13C2.99805 7.47715 7.4752 3 12.998 3C14.998 3 16.998 4 20.998 3Z"></path>
        </svg>
        <h1 className="text-2xl font-extrabold text-primary">RiceCare AI</h1>
      </div>

      <div className="right">
        <ul className="flex gap-8 ">
          <NavLink to="/" className={linkStyle}>Home</NavLink>
          <NavLink to="/detect" className={linkStyle}>Detect Disease</NavLink>
          <NavLink to="/about" className={linkStyle}>About</NavLink>
          <NavLink to="/contact" className={linkStyle}>Contact</NavLink>
        </ul>
      </div>
    </nav>
  );
};

export default Navbar;
